function removeItem(e) {
    ...
    Swal.fire({
        ...
        if (result.isConfirmed) {
            var totalProduct = parseInt($("#total-item").text());

            var node = $($($(e).parent().get(0)).children()).val();

            var nodeEmptyProduct = '<li class="item" id="nullItem"><h3> Không có sản phẩm nào</h3></li>';

            $.ajax({
                url: "/Cart/RemoveCartItem",
                method: "DELETE",
                async: true,
                data: { productDetailId: node },
                success: function (data) {
                    switch (data) {
                        case "Success":
                            ...
                            $("#total-item").text(totalProduct.toString() + " items");

                            if (totalProduct === 0) {
                                $("#cart-sidebar").append(nodeEmptyProduct);
                            }
